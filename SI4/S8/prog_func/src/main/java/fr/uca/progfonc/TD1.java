...
public class TD1 {
    ...
    public static <T extends Comparable<T>> T max(Lst<T> list) {
        if (list.cdr() == null) {
            return list.car();
        }
        T max = max(Objects.requireNonNull(list.cdr()));
        return list.car().compareTo(max) > 0 ? list.car() : max;
    }
}
